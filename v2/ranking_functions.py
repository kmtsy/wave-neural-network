...
def get_avg_torque(torque_curve, r1, r2):
    lb = np.where(torque_curve[:,4] == r1)[0]
    ub = np.where(torque_curve[:,4] == r2)[0]
    shape = torque_curve.shape
    avg_torque = np.sum(torque_curve[lb[0]:ub[0], 5])/(ub[0]/lb[0])  # finding the avg torque of the curve
